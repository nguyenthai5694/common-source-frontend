...
process.env.NODE_ENV !== 'production' ? DataGridPremiumRaw.propTypes = {
  ...
  /**
   * If `true`, the pageSize is calculated according to the container size 
   * and the max number of rows to avoid rendering a vertical scroll bar.
   * @default false
   */
  ...
  /**
   * If `true`, the "Select All" header checkbox selects only the rows 
   * on the current page. To be used in combination with `checkboxSelection`.
  ...
  checkboxSelectionVisibleOnly: chainPropTypes(PropTypes.bool, props => {
    if (!props.pagination && props.checkboxSelectionVisibleOnly) {
      return new Error('MUI: The `checkboxSelectionVisibleOnly` prop has no' +
        ' effect when the pagination is not enabled.');
    }
   ...
   * then the feature is fully disabled, and neither property nor method calls will have any effect.
   */
   ...
   * @returns {number | null} The estimated row height value. If `null` or 
   * `undefined` then the default row height, based on the density, is applied.
   */
   ...
   * @param {GridRowHeightParams} params With all properties from [[GridRowHeightParams]].
   * @returns {GridRowHeightReturnValue} The row height value. If `null` or `undefined` 
   ...
   */
  rowSelectionModel: PropTypes.oneOfType([PropTypes.arrayOf(
    PropTypes.oneOfType([PropTypes.number, PropTypes.string]).isRequired), PropTypes.number, PropTypes.string],
  ),
  /**
   ...
   */
  sx: PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.oneOfType(
    [PropTypes.func, PropTypes.object, PropTypes.bool])), PropTypes.func, PropTypes.object],
  ),
  /**
   * If positive, the Grid will throttle updates coming from `apiRef.current.updateRows` and `apiRef.current.setRows`.
   * It can be useful if you have a high update rate but do not want 
   * to do heavy work like filtering / sorting or rendering on each  individual update.
   * @default 0
   */
   ...
   * @param {GridCellSelectionModel} cellSelectionModel Object in the shape of 
   * [[GridCellSelectionModel]] containing the selected cells.
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
