...
import {
  useGridApiEventHandler, useGridApiMethod, GRID_ACTIONS_COLUMN_TYPE,
  GRID_CHECKBOX_SELECTION_COL_DEF, GRID_DETAIL_PANEL_TOGGLE_FIELD,
  gridRowsDataRowIdToIdLookupSelector, gridClasses, gridFocusCellSelector,
...
export const cellSelectionStateInitializer = (state, props) => {
  ...
  return _extends({}, state, {
    cellSelection: _extends({}, (_props$unstable_cellS = props.unstable_cellSelectionModel) != null ?
      _props$unstable_cellS : (_props$initialState = props.initialState) == null ?
        void 0 : _props$initialState.cellSelection),
  });
};
...
export const useGridCellSelection = (apiRef, props) => {
  ...
  const getSelectedCellsAsArray = React.useCallback(() => {
    ...
    return Object.entries(model).reduce((acc, [id, fields]) =>
      [...acc, ...Object.entries(fields).reduce((acc2, [field, isSelected]) => {
  ...
  const handleCellMouseDown = React.useCallback((params, event) => {
    ...
    (_apiRef$current$rootE = apiRef.current.rootElementRef) == null ?
      void 0 : (_apiRef$current$rootE2 = _apiRef$current$rootE.current) == null ?
        void 0 : _apiRef$current$rootE2.classList.add(gridClasses['root--disableUserSelection']);
  ...
  const handleCellMouseUp = React.useCallback(() => {
    ...
    (_apiRef$current$rootE3 = apiRef.current.rootElementRef) == null ?
      void 0 : (_apiRef$current$rootE4 = _apiRef$current$rootE3.current) == null ?
        void 0 : _apiRef$current$rootE4.classList.remove(gridClasses['root--disableUserSelection']);
  }, [apiRef]);
