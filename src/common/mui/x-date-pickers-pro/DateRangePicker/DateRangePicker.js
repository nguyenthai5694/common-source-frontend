...
process.env.NODE_ENV !== 'production' ? DateRangePicker.propTypes = {
  ...
  /**
   * If `true`, disable values after the current date for date components, 
   * time for time components and both for date time components.
   * @default false
   */
  ...
  /**
   * If `true`, disable values before the current date for date components, 
   * time for time components and both for date time components.
   * @default false
   */
   ...
   * @template TValue The value type. Will be either the same 
   * type as `value` or `null`. Can be in `[start, end]` format in case of range value.
