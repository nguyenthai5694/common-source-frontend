...
import {
  GridBody, GridFooterPlaceholder, GridHeader, GridRoot,
  GridContextProvider, useGridSelector,
...
DataGridProRaw.propTypes = {
  ...
  /**
   * If `true`, the pageSize is calculated according to the container 
   * size and the max number of rows to avoid rendering a vertical scroll bar.
   * @default false
   */
   ...
   * @param {GridRowHeightParams} params With all properties from [[GridRowHeightParams]].
   * @returns {GridRowHeightReturnValue} The row height value. If `null` or 
   * `undefined` then the default row height is applied. If "auto" 
   * then the row height is calculated based on the content.
   */
   ...
   */
  sx: PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.func, PropTypes.object, PropTypes.bool])),
  PropTypes.func, PropTypes.object]),
  /**
   * If positive, the Grid will throttle updates coming from `apiRef.current.updateRows` and `apiRef.current.setRows`.
   * It can be useful if you have a high update rate but do not want to do heavy work like 
   * filtering / sorting or rendering on each  individual update.
   * @default 0
   */
