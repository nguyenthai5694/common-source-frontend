...
export function getDataKey(rawKey: string) {
  return `pageData:${getCurrentRouteId()}:${rawKey}`;
}

export function getCustomDataKey(rawKey: string, path: string) {
  return `pageData:${path}:${rawKey}`;
}
